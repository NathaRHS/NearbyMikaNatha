...
CREATE TABLE modeles (
    ...
    CONSTRAINT fk_modeles_category
...
CREATE TABLE products (
    ...
    CONSTRAINT fk_products_image
        FOREIGN KEY (id_product_image) REFERENCES product_images(id)
        ON DELETE SET NULL,
...
CREATE TABLE size_modele (
    id SERIAL PRIMARY KEY,
    id_modele INT NOT NULL,
    id_size INT NOT NULL,
    CONSTRAINT fk_size_modele_modele
        FOREIGN KEY (id_modele) REFERENCES modeles(id)
        ON DELETE CASCADE,
    CONSTRAINT fk_size_modele_size
        FOREIGN KEY (id_size) REFERENCES sizes(id)
        ON DELETE CASCADE,
    CONSTRAINT uq_size_modele UNIQUE (id_modele, id_size)
);
